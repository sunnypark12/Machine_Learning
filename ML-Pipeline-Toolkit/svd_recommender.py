...
class SVDRecommender(object):
    ...
    def predict(self, R: np.ndarray, U_k: np.ndarray, V_k: np.ndarray,
        ...
        rating = R[idx, :]
        watched = set(np.where(~np.isnan(rating))[0])
        movs = [
            (np.dot(U_k[idx, :], V_k[:, movies_index[self.get_movie_id_by_name(movie_name)]]), movie_name)
            for movie_name in movies_pool
            if movies_index[self.get_movie_id_by_name(movie_name)] not in watched
        ]
        movs.sort(reverse=True, key=lambda x: x[0])
        return np.array([movie for _, movie in movs[:top_n]])
