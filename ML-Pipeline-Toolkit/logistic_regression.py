...
class LogisticRegression(object):
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray, x_val: np.
		...
		"""
        x_aug_train = self.bias_augment(x_train)
        x_aug_val = self.bias_augment(x_val)

        theta = np.zeros((x_aug_train.shape[1], 1))

        for epoch in range(epochs):
            h_x = self.predict_probs(x_aug_train, theta)
            grad = self.gradient(x_aug_train, y_train, h_x)
            theta -= lr * grad
